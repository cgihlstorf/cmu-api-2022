...
def regex_extractor(python_file: str, api_name: str) -> list: #api_name is either 'tf' (for Tensorflow) or 'pt' (For PyTorch)
    ...
    if api_name == "tf":
        ...
        print("Invalid entry to function regex_extractor(). Must be either 'tf' (for Tensorflow) or 'pt' (For PyTorch)") 

    #Compile a list of all relevant function calls
    for line in file_lines:
        line_list = re.findall(regex, line)
        if line_list != []: 
            return_list.append(line_list[0])
...
def AST_extractor(python_file: str, api_name: str):
